...
export default function ModularDashboard({ zoneId, immersionMode, setImmersionMode, openLightingPanel }: ModularDashboardProps) {
  const { activeColors } = useTheme();
  
  // Hardware configuration for this specific zone
  const zoneConfig = useMemo<ZoneHardwareConfig | null>(() => {
    const config = getAircraftConfig();
    return config.zones.find(z => z.id === zoneId) || null;
  }, [zoneId]);

  // Compute default layout for this zone based on its hardware
  const defaultZoneLayout = useMemo(() => {
    return masterLayout.filter(item => {
      if (!zoneConfig) return true; // Fallback
      if (item.i === 'media') return zoneConfig.hasMonitors;
      if (item.i === 'audio') return zoneConfig.hasSpeakers;
      if (item.i === 'climate') return zoneConfig.hasClimate;
      if (item.i === 'shades') return zoneConfig.hasShades;
      if (item.i === 'lighting') return zoneConfig.hasLighting;
      return true; // Map, Timeline, Data, Environment, Seat, Service always exist for now
    });
  }, [zoneConfig]);
  ...
  }, [zoneId, defaultZoneLayout]);

  const onLayoutChange = (newLayout: any) => {
    setLayout(newLayout);
    localStorage.setItem(`tahquitz-dashboard-layout-${zoneId}`, JSON.stringify(newLayout));
  };

  const resetLayout = () => {
    setLayout(defaultZoneLayout);
    localStorage.removeItem(`tahquitz-dashboard-layout-${zoneId}`);
  };
  ...
  return (
    <div id="dashboard-container" className={`w-full h-full pb-24 ${isEditing ? 'editing-mode' : ''}`}>
      <div className="w-full flex justify-between items-center mb-6 px-2">
        <h2 className="text-xl font-light tracking-[0.2em] text-white/80 uppercase">
          YOUR SPACE {zoneConfig && !zoneConfig.hasMonitors ? '(AUDIO ONLY)' : ''}
        </h2>
      ...
      >
        ...
        {/* MEDIA GALLERY (Conditional) */}
        {(!zoneConfig || zoneConfig.hasMonitors) && (
        ...
        {(!zoneConfig || zoneConfig.hasSpeakers) && (
          <div key="audio" className={`relative group ${isEditing ? 'ring-2 ring-white/50 rounded-3xl animate-pulse' : ''}`}>
            {isEditing && (
        ...
        {/* CLIMATE WIDGET (Conditional) */}
        {(!zoneConfig || zoneConfig.hasClimate) && (
        ...
        {/* LIGHTING MASTER (Conditional) */}
        {(!zoneConfig || zoneConfig.hasLighting) && (
        ...
        {/* WINDOW SHADES (Conditional) */}
        {(!zoneConfig || zoneConfig.hasShades) && (
